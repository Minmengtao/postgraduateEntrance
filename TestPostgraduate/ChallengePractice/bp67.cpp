//经过一个点时，圆心走的是圆弧路线，经过两条切线时直接转向，不需要考虑圆心走向
/*#include<stdio.h>
#include<math.h>
typedef struct Dot{
	double x;
...
Dot d[105];
int n;
double cir[105],dir[105],r,PI=3.141593,sum;
int main(){
	while(scanf("%d %lf",&n,&r)!=EOF){
		...
		for(i=1;i<n;i++){
			sum+=sqrt((d[i].x-d[i-1].x)*(d[i].x-d[i-1].x)+(d[i].y-d[i-1].y)*(d[i].y-d[i-1].y));
			if(d[i].y==d[i-1].y&&d[i].y==d[i+1].y)
				continue;
			if(d[i].y>=d[i-1].y&&d[i].y>=d[i+1].y&&i<n-1){
				double a=(d[i].x-d[i-1].x)*(d[i].x-d[i-1].x)+(d[i].y-d[i-1].y)*(d[i].y-d[i-1].y);
				double b=(d[i].x-d[i+1].x)*(d[i].x-d[i+1].x)+(d[i].y-d[i+1].y)*(d[i].y-d[i+1].y);
				double c=(d[i+1].x-d[i-1].x)*(d[i+1].x-d[i-1].x)+(d[i+1].y-d[i-1].y)*(d[i+1].y-d[i-1].y);
				cir[len1++]=r*(PI-acos((a+b-c)/(2*sqrt(a)*sqrt(b))));
			}
			if(d[i].y<=d[i-1].y&&d[i].y<=d[i+1].y&&i<n-1){
				double a=(d[i].x-d[i-1].x)*(d[i].x-d[i-1].x)+(d[i].y-d[i-1].y)*(d[i].y-d[i-1].y);
				double b=(d[i].x-d[i+1].x)*(d[i].x-d[i+1].x)+(d[i].y-d[i+1].y)*(d[i].y-d[i+1].y);
				double c=(d[i+1].x-d[i-1].x)*(d[i+1].x-d[i-1].x)+(d[i+1].y-d[i-1].y)*(d[i+1].y-d[i-1].y);
				dir[len2++]=2*r/tan(acos((a+b-c)/(2*sqrt(a)*sqrt(b)))/2);
			}
		}
		for(i=0;i<len1;i++)
			sum+=cir[i];
...
#include <iostream>  
 ...
 {  
     ...
     for (i = 0; i < (int)segs.size(); ++i)  
     {  
         for (j = i+1; j < (int)segs.size(); ++j)  
         {  
             segs[i]->intersect(segs[j]);  
             if (tIntersectRet > 0)  
             {  
                 for (id = 0; id < tIntersectRet; ++id)  
                 {  
